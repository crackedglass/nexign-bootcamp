...
class SubscriberManagementServiceTest {
    ...
    @BeforeEach
    void setUp() {
        existingSubscriber = new Subscriber(1L, "Existing User", "79111111111", BigDecimal.valueOf(50.0), LocalDate.now().minusDays(10), 11L);
    }

    @Test
    void createSubscriber_ShouldSucceed_WhenMsisdnDoesNotExist() {
        NewSubscriberRequestBrtDto request = new NewSubscriberRequestBrtDto("79222222222", "New User", 12L, BigDecimal.valueOf(100.0));
    ...
    void createSubscriber_ShouldThrowConflict_WhenMsisdnExists() {
        NewSubscriberRequestBrtDto request = new NewSubscriberRequestBrtDto("79111111111", "Another User", 11L, BigDecimal.valueOf(100.0));
        when(subscriberRepository.findByMsisdn("79111111111")).thenReturn(Optional.of(existingSubscriber));

        assertThrows(ResponseStatusException.class, () -> {
            subscriberManagementService.createSubscriber(request);
        });
    ...
    void getSubscriberByMsisdn_ShouldThrowNotFound_WhenNotFound() {
        ...
        assertThrows(ResponseStatusException.class, () -> {
            subscriberManagementService.getSubscriberByMsisdn("79999999999");
        });
    }
    ...
    void updateTariff_ShouldThrowNotFound_WhenNotFound() {
        ...
        assertThrows(ResponseStatusException.class, () -> {
            subscriberManagementService.updateTariff("79999999999", newTariffId);
        });
        verify(subscriberRepository, never()).save(any(Subscriber.class));
    }
    ...
    void topUpBalance_ShouldCallBillingService_WhenFound() {
        when(billingService.topUpBalance(eq("79111111111"), any(BigDecimal.class))).thenReturn(true);
    ...
    void topUpBalance_ShouldThrowException_WhenBillingServiceFails() {
        when(billingService.topUpBalance(eq("79111111111"), any(BigDecimal.class))).thenReturn(false);
        ...
        BigDecimal amount = BigDecimal.ZERO; // Example of invalid amount causing failure
    ...
    void topUpBalance_ShouldThrowNotFound_WhenBillingServiceFailsAndSubscriberNotFound() {
        when(billingService.topUpBalance(eq("79999999999"), any(BigDecimal.class))).thenReturn(false);
        when(subscriberRepository.findByMsisdn("79999999999")).thenReturn(Optional.empty()); // Subscriber doesn't exist
        BigDecimal amount = BigDecimal.TEN;

        ResponseStatusException exception = assertThrows(ResponseStatusException.class, () -> {
            subscriberManagementService.topUpBalance("79999999999", amount);
        });
