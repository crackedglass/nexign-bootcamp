...
public class RatingService {
        ...
        try {
            ...
            if (tariff.getTariffType() == Tariff.TariffType.CLASSIC) {
                ...
                if (callDurationMinutes <= remainingIncludedMinutes) {
                    cost = BigDecimal.ZERO; // Covered by package
    ...
    private BigDecimal calculateClassicCost(CallDataDto callData, Tariff classicTariff, long callDurationMinutes) {
        ...
        return rate.multiply(BigDecimal.valueOf(callDurationMinutes)).setScale(2, RoundingMode.HALF_UP);
    }
    ...
    private void sendResponse(Long brtCallDataId, String msisdn, BigDecimal cost, boolean success, String errorMessage) {
        RatingResponseDto response = new RatingResponseDto(brtCallDataId, msisdn, cost, success, errorMessage);
        try {
            rabbitTemplate.convertAndSend(hrsExchangeName, responseRoutingKey, response);
            log.info("Sent rating response for MSISDN {}: Cost={}, Success={}, BRTCallDataId={}", msisdn, cost, success, brtCallDataId);
        } catch (Exception e) {
    ...
    private int getMinutesUsedThisMonth(Long brtSubscriberId, YearMonth month) {
        ...
        return usageOpt.map(SubscriberMonthlyUsage::getUsedMinutes).orElse(0);
    }
    ...
    private void updateMonthlyUsage(Long brtSubscriberId, YearMonth month, long minutesToAdd, Long tariffId) {
        ...
        usage.setUsedMinutes(usage.getUsedMinutes() + (int) minutesToAdd);
        monthlyUsageRepository.save(usage);
        log.info("Updated monthly usage for subscriberId {}, month {}: added {} minutes. New total: {}. TariffId: {}", 
                 brtSubscriberId, month, minutesToAdd, usage.getUsedMinutes(), tariffId);
    }
